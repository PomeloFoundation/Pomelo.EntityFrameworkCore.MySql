...
{
#if !Test21
    public class SpatialQueryMySqlGeometryTest : SpatialQueryTestBase<SpatialQueryMySqlGeometryFixture>
    {
        public SpatialQueryMySqlGeometryTest(SpatialQueryMySqlGeometryFixture fixture, ITestOutputHelper testOutputHelper)
            : base(fixture)
        {
...
FROM [GeoPointEntity] AS [e]");
        ...
        public override async Task Distance_on_converted_geometry_type_constant(bool isAsync)
        {
            ...
            AssertSql(
                @"SELECT [e].[Id], [e].[Location].STDistance(geometry::Parse('POINT (0 1)')) AS [Distance]
FROM [GeoPointEntity] AS [e]");
        }
        ...
        public override async Task Distance_on_converted_geometry_type_constant_lhs(bool isAsync)
        {
            ...
            AssertSql(
                @"SELECT [e].[Id], geometry::Parse('POINT (0 1)').STDistance([e].[Location]) AS [Distance]
FROM [GeoPointEntity] AS [e]");
        }
...
FROM [MultiLineStringEntity] AS [e]");
        ...
        public override async Task GetInteriorRingN(bool isAsync)
        {
            ...
            AssertSql(
                @"SELECT [e].[Id], CASE
    WHEN [e].[Polygon] IS NULL OR ([e].[Polygon].STNumInteriorRing() = 0)
    THEN NULL ELSE [e].[Polygon].STInteriorRingN(0 + 1)
