...
#if Test20
            var storeType = TestServiceFactory.Instance.Create<MySqlTypeMapper>().FindMapping(property).StoreType;
#else
            var storeType = TestServiceFactory.Instance.Create<MySqlTypeMappingSource>().GetMapping(property).StoreType;
#endif
