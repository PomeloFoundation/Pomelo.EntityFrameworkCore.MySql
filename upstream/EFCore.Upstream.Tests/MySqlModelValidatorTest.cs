...
namespace Microsoft.EntityFrameworkCore
{
    public class MySqlModelValidatorTest : RelationalModelValidatorTest
    {
        ...
        [Fact]
        public virtual void Passes_for_incompatible_foreignKeys_within_hierarchy_when_one_name_configured_explicitly_for_mySql()
        {
            var modelBuilder = CreateConventionalModelBuilder();
            modelBuilder.Entity<Animal>();
        ...
        [Fact]
        public virtual void Passes_for_incompatible_indexes_within_hierarchy_when_one_name_configured_explicitly_for_mySql()
        {
            var modelBuilder = CreateConventionalModelBuilder();
            modelBuilder.Entity<Animal>();
            ...
            Assert.Equal("IX_Animal_Name", index1.MySql().Name);
            Assert.Equal("IX_Animal_Name1", index2.Relational().Name);
            Assert.Equal("IX_Animal_Name1", index2.MySql().Name);
        }
        ...
        public virtual void Detects_default_decimal_mapping()
        {
            ...
            VerifyWarning(MySqlStrings.LogDefaultDecimalTypeColumn.GenerateMessage("Price", nameof(Animal)), modelBuilder.Model);
        }
        ...
        public void Detects_byte_identity_column()
        {
            ...
            VerifyWarning(MySqlStrings.LogByteIdentityColumn.GenerateMessage("Bite", nameof(Dog)), modelBuilder.Model);
        }
        ...
        public void Throws_for_duplicate_include_properties()
        {
            var modelBuilder = InMemoryTestHelpers.Instance.CreateConventionBuilder();
            modelBuilder.Entity<Dog>().Property(c => c.Type);
            modelBuilder.Entity<Dog>().HasIndex(nameof(Dog.Name)).ForMySqlInclude(nameof(Dog.Type), nameof(Dog.Type));

            VerifyError(MySqlStrings.IncludePropertyDuplicated(nameof(Dog), nameof(Dog.Type)), modelBuilder.Model);
        }
